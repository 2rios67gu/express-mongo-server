...
mongoose
    ...
    .catch((err) => console.log("Error al conectar la base datos", err));
...
app.put("/papa/:id", async(req, res) => {
    try{
        ...
        (req.params,id,
        req.body,
        { new: true, runValoidators: true }
        );
        if (!updatedPapa) return res.status(404).json({error: "Papa no encontrado"});
        res.json(updatedPapa);
    } catch (error) {
        res.status(404).json({ error: "Error al actualizar el papa", error });
    }
});
...
app.delete("/papa/:id", async(req, res) => {
    try{
        ...
        res.status(404).json({ error: "Error al eliminar el papa", error });
    }
});